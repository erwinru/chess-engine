...
class Game:
    def __init__(self, starting_player=True):
        ...
        inp = input("To play vs AI press x. To play vs yourself press y\n")
        if inp == "x":
            self.game_loop_ai(b)
        elif inp == "y":
            self.game_loop(b)
    ...
    def create_pieces(self):
        ...
        for i in range(8):
            ps += [
                p.Pawn(
                    (i, 6), color=True, value=1, image=load("./images/w_pawn.png"),
                ),
                p.Pawn(
                    (i, 1), color=False, value=-1, image=load("./images/b_pawn.png"),
                ),
            ]
        return ps
            ...
            for event in pg.event.get():
                ...
                if self.is_player_white:
                    if event.type == pg.MOUSEBUTTONDOWN:
                        mx_start, my_start = pg.mouse.get_pos()
                        pos_start = self.get_board_pos(mx_start, my_start)
                        if not b.is_empty(pos_start):
                            moved_piece = b.board[-1][
                                pos_start[1], pos_start[0]
                            ]

                    if event.type == pg.MOUSEBUTTONUP:
                        mx_end, my_end = pg.mouse.get_pos()
                        pos_end = self.get_board_pos(mx_end, my_end)
                        if (not b.is_empty(pos_start)) & self.right_player(
                            moved_piece
                        ):
