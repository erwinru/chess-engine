...
class Board:
    ...
    def print_piece_positions(self):
        for piece in self.get_pieces():
            print(
                "{}: -- > {}".format(
                    piece, self.change_notation(piece.pos)
                )
            )
    ...
    def check_castled(self, piece, pos_start, pos_end):
        if isinstance(piece, p.King):
            ...
            if pos_diff == -2:
                self.board[-1][
                    king.rel_pos(1, 0)[1], king.rel_pos(1, 0)[0]
                ] = 0
                rook = p.Rook(
                    king.rel_pos(-1, 0),
                    color=king.color,
            ...
            # long castleing
                ...
                self.board[-1][
                    king.rel_pos(-2, 0)[1], king.rel_pos(-2, 0)[0]
                ] = 0
                rook = p.Rook(
                    king.rel_pos(1, 0),
                    color=king.color,
    ...
    def get_pieces(self):
        return [
            field for field in self.board[-1].reshape(64) if field != 0
        ]
